...
# Spatial dimension
num_underlyings = 1
num_features = num_underlyings
# num_features = 2 * num_underlyings


# Spatial domain of interest at t=0 (hyperrectangle)
# spot_min = tf.zeros(dim, dtype=DTYPE)
# spot_max = tf.ones(dim, dtype=DTYPE)
spot_min = 90 * tf.ones(num_underlyings, dtype=DTYPE)
spot_max = 110 * tf.ones(num_underlyings, dtype=DTYPE)
...
sigma = tf.constant(test_vol, dtype=DTYPE)
...
###################################################################################################
# Original experiment
model = init_model(dim_=num_underlyings, num_hidden_neurons=200)
...
num_disp = 100
disp_spot = np.linspace(spot_min * 1.0, spot_max * 1.0, num_disp)
x_disp = np.ndarray((num_disp, num_features))
for disp in range(num_disp):
    disp_point = np.ndarray(num_features)
    for u in range(num_underlyings):
        disp_point[u] = disp_spot[disp]

    x_disp[disp] = disp_point
...
for disp in range(num_disp):
    fwd = x_disp[disp] * np.exp((repo_rate - div_rate) * expiry)
    y_ref[disp] = df * black_formula(fwd, strike, test_vol, expiry, True)

# Calculate model
# if train:
y_mod = model.predict(x_disp)
...
# # plt.scatter(disp_spot, y_ref, color='blue', alpha=0.4, label='Full series')
# x_scat = x_set[:, 0]
# plt.scatter(x_scat, y_set, color='blue', alpha=0.1, label='Full series')
# plt.legend(loc='upper right')
# plt.show()
