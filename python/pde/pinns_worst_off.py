...
@tf.function
def train_step(model_, optimizer_, pde_points_, boundary_points_, boundary_values_):
    # Compute current loss and gradient
    loss_, grad_, pde_loss_, payoff_loss_, side_loss_ = calculate_loss_grad(model_, pde_points_,
                                                                            boundary_points_,
                                                                            boundary_values_)
...
# def train_step():
#     # Compute current loss and gradient
#     loss_, grad_, pde_loss_, payoff_loss_, side_loss_ = calculate_loss_grad(model, pde_points,
#                                                                             boundary_points,
#                                                                             boundary_values)
#
#     # Apply gradient descent
...
# Launch training
# @tf.function
def launch_training(model_names_, model_rmses_, model_runtimes_):
    # ################ Initialization ######################################################################
    ...
    for epoch in range(num_epochs + 1):
        ...
        loss, pde_loss, payoff_loss, side_loss = train_step(model, optimizer, pde_points, boundary_points,
                                                            boundary_values)
        ...
        if use_best_model and epoch % 100 == 0 and loss < best_loss_to_date:
    ...
    # rmse_ = np.sqrt(sk.metrics.mean_squared_error(mc_pv, md_pv))
    # model_names_.append(this_model_name)
    # model_rmses_.append(rmse_)
    # model_runtimes_.append(runtime)
...
# for run in range(3):
launch_training(model_names, model_rmses, model_runtimes)

launch_training(model_names, model_rmses, model_runtimes)

# launch_training(model_names, model_rmses, model_runtimes)

# launch_training(model_names, model_rmses)

# Output to csv
model_perfs = {'Name': model_names, 'Rmse': model_rmses, 'Runtime': model_runtimes}
model_perf_df = pd.DataFrame(model_perfs)
model_perf_file = os.path.join(model_folder, model_name + "_stats.tsv")
model_perf_df.to_csv(model_perf_file, sep="\t", index=False)
