...
def plot_hist_returns(basket_x_days_hist_rtns, df_buy_sell_signal, x_lim_tuple, z_score_threshold, vertline_date):
    ...
    #-------------------------------------------------------------------------
    today_ = basket_x_days_hist_rtns.index[-1].strftime('%Y-%m-%d')
    dates_to_buy, retns_to_buy, dates_to_sell, retns_to_sell = ct.extract_data_conditions_on_buy_sell_signal(
...
# x_lim_tuple contains (start_date, end_date)
def plot_two_time_series_with_limits(time_series, x_lim_tuple, y_lim_tuple, vertline_date,
                                     num_days = settings.HOLDING_PERIOD_IN_DAYS):
...
# x_lim_tuple contains (start_date, end_date)
def plot_trading_signal_graphs(time_series_basket, basket_mean, basket_std, df_buy_sell_signal, x_lim_tuple,
                               y_lim_tuple, vertline_date):
    ...
    #-------------------------------------------------------------------------
    today_ = time_series_basket.index[-1].strftime('%Y-%m-%d')
    dates_to_buy, retns_to_buy, dates_to_sell, retns_to_sell = ct.extract_data_conditions_on_buy_sell_signal(
        time_series_basket, df_buy_sell_signal, today_)
...
# x_lim_tuple contains (start_date, end_date)
def plot_is_still_coint_graphs(time_series_basket, basket_mean, basket_std, df_still_coint, x_lim_tuple, y_lim_tuple,
                               vertline_date):
