...
class SDevPyCallback(keras.callbacks.Callback):
    """ SDevPy's base class for Keras callbacks. Compared to Keras's base, it also keeps track of
    ...
    def __init__(self, epoch_sampling=1, optimizer=None, loss_function=None, x_train=None, y_train=None):
        ...
        self.learning_rates = []
        self.sampled_epochs = []
        # Used to calculate the loss periodically, as in reality the loss function is used on the transformed
        # coordinates, not, as the user might expect, on the real world ones.
...
class RefCallback(SDevPyCallback):
    """ Compares model predictions vs reference periodically """
